  ...
  useEffect(() => {
    const fetchCartItems = async () => {
      try {
        ...
        const response = await axios.get(`https://0ec3-2400-adc5-453-1500-907c-6c3d-f922-3664.ngrok-free.app/api/cart/get?userId=${userId}`, {
          ...
          <ul className="cart-items">
            {cart.items.map((item) => (
              <li key={item.id} className={`cart-item ${item.status === 'Completed' ? 'completed' : ''}`}>
                <img src={`https://0ec3-2400-adc5-453-1500-907c-6c3d-f922-3664.ngrok-free.app${item.Product.productImage}`} alt={item.Product.productName} className="product-image" />
